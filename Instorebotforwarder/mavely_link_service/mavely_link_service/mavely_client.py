...
def _build_cookie_header(session_token_or_cookie: str) -> str:
    """
    Accepts either:
    - the raw next-auth session-token VALUE (recommended for the modal), OR
...
class MavelyClient:
    def __init__(
        ...
        # This lets you fix/rotate auth once (in api-token.env) without editing guild config.
        ...
        self.auth_audience = _env_str("MAVELY_AUTH_AUDIENCE")  # optional, if API requires specific audience
        self.auth_scope = _env_str("MAVELY_AUTH_SCOPE")        # optional
        self.client_id = _env_str("MAVELY_CLIENT_ID")
        self._last_refresh_error: Optional[str] = None
        # IMPORTANT: cookie token must match the domain you are calling.
        # Default to creators.joinmavely.com because that is where most sessions live now.
        ...
        # Browser fingerprinting: allow overriding these via env to match your real Chrome request.
        self.user_agent = (user_agent or _env_str("MAVELY_USER_AGENT")) or (
            "Mozilla/5.0 (Windows NT 10.0; Win64; x64) "
            "AppleWebKit/537.36 (KHTML, like Gecko) "
            "Chrome/144.0.0.0 Safari/537.36"
        )
        # .env quoting is easy to mess up for sec-ch-ua; provide safe defaults matching your Chrome capture.
    ...
    def _refresh_access_token(self, sess: requests.Session) -> Optional[str]:
        ...
        try:
            resp = sess.post(
                self.token_endpoint,
                # OAuth token endpoints are typically form-encoded (Auth0-compatible)
                headers={"Content-Type": "application/x-www-form-urlencoded", "Accept": "application/json"},
                data=payload,
                timeout=self.timeout_s,
            )
        except requests.RequestException as e:
            self._last_refresh_error = f"refresh request failed: {e}"
            self.log.info("Refresh request failed: %s", e)
            return None

        ct = (resp.headers.get("content-type") or "").lower()
        self.log.info("Refresh response: status=%s ct=%s", resp.status_code, ct)
        if resp.status_code != 200:
            # Try to extract safe error details from JSON response
            err_code = ""
            err_desc = ""
            try:
                j = resp.json() if "application/json" in ct else _parse_json_best_effort(resp.text or "")
                if isinstance(j, dict):
                    err_code = str(j.get("error") or j.get("code") or j.get("name") or "").strip()
                    err_desc = str(j.get("error_description") or j.get("message") or j.get("description") or "").strip()
            except Exception:
                pass
            extra = ""
    ...
    def preflight(self) -> MavelyResult:
        ...
        # Avoid hammering OAuth refresh on startup unless explicitly enabled (refresh tokens may be invalid/rotated).
        ...
        if not self.cookie_header:
            # Cookie-less mode: nothing else to validate here.
        ...
        # Treat empty session JSON as "not authenticated" (otherwise logs say ok=True and users get misled).
        data: object = None
        if ok:
            try:
                data = resp.json()
        ...
        # Optionally auto-derive bearer token from session JSON if it's present (no logging of body).
    ...
    def _session_headers(self) -> Dict[str, str]:
        ...
        headers = {
            # Browser request is typically */* for this endpoint.
            "Accept": "*/*",
    ...
    def _ensure_auth_token_from_session(self, sess: requests.Session, *, force: bool = False) -> bool:
        """
        Fetch NextAuth session JSON and extract the "token" field (bearer access token).
        Returns True if we updated auth_token, else False.

        If force=False (default), this only runs when auth_token is empty.
        """
        ...
        if _is_empty_session_json(data):
            # This is the most common cause of "TOKEN_EXPIRED forever": cookies are no longer logged in.
            if force:
                self.log.info(
                    "Mavely: session endpoint returned empty JSON. Your MAVELY_COOKIES are not logged in anymore."
                )
            return False
        ...
        # Help debug why session is empty (without dumping secrets)
        try:
    ...
    def _headers(self) -> Dict[str, str]:
        # These headers intentionally look like a real browser request.
        origin = self.base_url
    ...
    def _graphql_headers_cookie_only(self) -> Dict[str, str]:
        # Same as _graphql_headers(), but explicitly omits Authorization.
        origin = self.base_url
        referer = f"{self.base_url}/tools"
        headers = {
            "Content-Type": "application/json",
    ...
    def _try_graphql_create_link(self, sess: requests.Session, url: str) -> Optional[MavelyResult]:
        """
        Prefer GraphQL if available; this matches the browser response shape:
          { "data": { "createAffiliateLink": { "link": "https://mavely.app.link/..." } } }
        """
        if not self.auth_token and not self.cookie_header:
            return None

        # If no explicit bearer token, attempt to derive it from the NextAuth session JSON.
        ...
        self.log.debug(
            "Browser headers: ua=%r sec-ch-ua=%s sec-ch-ua-platform=%s",
            self.user_agent,
            bool(self.sec_ch_ua),
            bool(self.sec_ch_ua_platform),
        )
        query = """
...
""".strip()
        ...
        if "text/html" in ct:
            return MavelyResult(
                ...
                error="Blocked or not authenticated (HTML response from GraphQL).",
                raw_snippet=snippet,
            )

        if resp.status_code != 200:
            last_err: Optional[MavelyResult] = None
            # If access token is rejected, try idToken (if provided) once.
            if resp.status_code == 401 and self.id_token and self.id_token != self.auth_token:
                ...
                try:
                    ...
                    self.log.debug("GraphQL(idToken) response: status=%s ct=%s", resp2.status_code, ct2)
                    ...
                    # Don't return yet: allow refresh-token fallback after idToken rejection.
                    ...
                    # Don't return yet: allow refresh-token fallback.
                    last_err = MavelyResult(ok=False, status_code=0, error=str(e))

            # If still unauthorized, try refreshing an access token (if refresh token is available) and retry once.
                ...
                if enable_oauth_refresh:
                    ...
                    if refreshed:
                        ...
                        try:
                            ...
                            self.log.debug("GraphQL(refreshed) response: status=%s ct=%s", resp3.status_code, ct3)
                            if resp3.status_code == 200:
                                ...
                                if link3:
                                    # Update in-memory token for next call
        ...
        try:
            ...
            data = _parse_json_best_effort(resp.text or "")
        if not isinstance(data, dict):
            self.log.info("GraphQL JSON parse failed (ct=%s body_len=%s snippet=%r)", ct, len(resp.text or ""), snippet[:120])
            return MavelyResult(ok=False, status_code=200, error="Failed to parse GraphQL JSON response", raw_snippet=snippet)

        # Some deployments wrap GraphQL responses as: {"response": {"errors": [...], "data": {...}, "status": 200}, "request": {...}}
        ...
        if isinstance(payload, dict) and errors:
            # If token is expired, try refreshing once (if refresh token is available), then retry.
            if _is_token_expired_error(errors):
                # 0) Sometimes cookies alone can still authorize GraphQL even if a cached bearer token is expired.
                try:
                    ...
                    if resp_cookie.status_code == 200:
                        ...
                        if isinstance(data_cookie, dict):
                            payload_cookie = (
                                data_cookie.get("response")
                                if isinstance(data_cookie.get("response"), dict)
                                else data_cookie
                            )
                            errors_cookie = (
                                (payload_cookie or {}).get("errors")
                                if isinstance(payload_cookie, dict)
                                else None
                            )
                ...
                # Try idToken first (often lasts longer than the access token).
                if self.id_token and self.id_token != self.auth_token:
                    ...
                    try:
                        ...
                        ct_id = (resp_id.headers.get("content-type") or "").lower()
                        ...
                        if resp_id.status_code == 200:
                            ...
                            if isinstance(data_id, dict):
                                ...
                                if isinstance(payload_id, dict) and errors_id:
                                    # still errors; fall through to other recovery
                                    pass
                                else:
                ...
                # Primary: try to re-derive a fresh bearer from /api/auth/session using cookies.
                if self._ensure_auth_token_from_session(sess, force=True):
                    self.log.debug("GraphQL token expired; re-derived bearer from session. Retrying GraphQL.")
                    try:
                        resp4 = _do_post(self._graphql_headers())
                        ct4 = (resp4.headers.get("content-type") or "").lower()
                        snippet4 = (resp4.text or "")[:500]
                        self.log.debug("GraphQL(session-refreshed) response: status=%s ct=%s", resp4.status_code, ct4)
                ...
                # Secondary: OAuth refresh (if configured)
                enable_oauth_refresh = (_env_str("MAVELY_ENABLE_OAUTH_REFRESH") or "").strip().lower() in {"1", "true", "yes", "y", "on"}
                if enable_oauth_refresh:
                    refreshed = self._refresh_access_token(sess)
                    if refreshed:
                        self.log.debug("GraphQL token expired; retrying with refreshed access token")
                        try:
                            resp5 = _do_post(self._graphql_headers_with_bearer(refreshed))
                            ct5 = (resp5.headers.get("content-type") or "").lower()
                            snippet5 = (resp5.text or "")[:500]
                            self.log.debug("GraphQL(refreshed-expired) response: status=%s ct=%s", resp5.status_code, ct5)
                ...
                # Refresh token missing or refresh failed
                ...
                return MavelyResult(
                    ok=False,
                    status_code=200,
                    error=f"Mavely token expired. {reason}.",
                    raw_snippet=snippet,
                )
        ...
        try:
            link_obj = (payload or {}).get("data", {}).get("createAffiliateLink", {}) if isinstance(payload, dict) else {}
    ...
    def create_link(self, url: str) -> MavelyResult:
        ...
        for attempt in range(1, self.max_retries + 1):
            self.rate.wait()
            try:
                # First try GraphQL (matches what your browser uses in many cases)
                self.log.debug(
                ...
                if gql_res and gql_res.ok and gql_res.mavely_link:
                    return gql_res
                # If GraphQL returned a real response (even an error), prefer it.
                # joinmavely.com returns 405 HTML on the legacy tRPC path, so falling back is misleading.
                if gql_res and gql_res.status_code not in (0, 404):
                    return gql_res

                # If we're on joinmavely.com and GraphQL didn't work, do NOT fall back to tRPC.
                # The tRPC path is not supported there (observed 405 + HTML).
