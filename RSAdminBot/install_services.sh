...
units=(
  ...
  "mirror-world-rschannelrelay.service"
...
for unit in "mirror-world-rsforwarder.service" "mirror-world-rsonboarding.service" "mirror-world-rscheckerbot.service" "mirror-world-rsmentionpinger.service" "mirror-world-rssuccessbot.service" "mirror-world-rspromobot.service" "mirror-world-rscashoutbot.service" "mirror-world-rschannelrelay.service"; do
...
echo "NOTE: RS Channel Relay: ensure RSChannelRelay/config.secrets.json has discord_bot_token, Message Content Intent ON in Dev Portal, then: sudo systemctl restart mirror-world-rschannelrelay.service"
...
echo "      Instorebotforwarder trusted Chrome (CDP @127.0.0.1:9222 for eBay scrapes; only needed when MWBots/Instorebotforwarder/config.json sets ebay_first8_connect_cdp:true):"
echo "        sudo apt-get install -y xvfb google-chrome-stable   # one-time"
echo "        sudo systemctl restart mirror-world-instorebotforwarder-chrome-cdp.service"
echo "        # Warm the profile once via noVNC if Akamai blocks the fresh profile:"
echo "        #   open noVNC, browse eBay normally, then restart this service."
