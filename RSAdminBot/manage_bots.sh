#!/bin/bash
# Bot Management Script for Mirror World
# Usage: ./manage_bots.sh <action> <bot_name>
# Actions: start, stop, restart, status
# Special bot_name: "all" or "allbots" to operate on all bots (excluding rsadminbot)
#
# This script can be used by RSAdminBot and RSForwarder to manage all bots
# including RSAdminBot itself.
# 
# NOTE: This script uses UNIQUE logic separate from ServiceManager:
# - Unmasks services if needed
# - Enables services if needed
# - Uses different verification timing
# - Does NOT interfere with normal bot starting via ServiceManager
...
# Service name mapping (matches RSAdminBot BOTS dictionary)
declare -A SERVICES=(
...
if [ "$BOT_NAME" = "all" ] || [ "$BOT_NAME" = "allbots" ]; then
    # Get all bots except rsadminbot
    ALL_BOTS=()
    for bot_key in "${!SERVICES[@]}"; do
        if [ "$bot_key" != "rsadminbot" ]; then
            ALL_BOTS+=("$bot_key")
        fi
    done
    
    # Execute action for all bots
    TOTAL=${#ALL_BOTS[@]}
    SUCCESS=0
    FAILED=0
    RESULTS=()
    ...
    for idx in "${!ALL_BOTS[@]}"; do
        ...
        # Recursively call this script for each bot
        if bash "$0" "$ACTION" "$bot_key" > /tmp/manage_bots_${bot_key}_$$.log 2>&1; then
            SUCCESS=$((SUCCESS + 1))
            RESULTS+=("✅ $bot_key: SUCCESS")
        else
            FAILED=$((FAILED + 1))
            ERROR=$(tail -3 /tmp/manage_bots_${bot_key}_$$.log 2>/dev/null | head -1 || echo "Unknown error")
            RESULTS+=("❌ $bot_key: FAILED - $ERROR")
        fi
        rm -f /tmp/manage_bots_${bot_key}_$$.log
    done
...
if [ -z "$SERVICE_NAME" ]; then
    ...
    echo "Available bots: ${!SERVICES[*]}"
    echo "Special: 'all' or 'allbots' to operate on all bots (excluding rsadminbot)"
    exit 1
fi
...
start_service() {
    ...
    # Unmask if needed (service might be masked)
