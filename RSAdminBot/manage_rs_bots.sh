#!/bin/bash
# RS Bots Management Script
# Usage: ./manage_rs_bots.sh <action> <bot_name>
...
# Service name mapping for RS bots
declare -A SERVICES=(
...
if [ "$BOT_NAME" = "all" ] || [ "$BOT_NAME" = "allbots" ]; then
    ...
    for idx in "${!ALL_BOTS[@]}"; do
        ...
        if bash "$0" "$ACTION" "$bot_key" > /tmp/manage_rs_bots_${bot_key}_$$.log 2>&1; then
            SUCCESS=$((SUCCESS + 1))
            RESULTS+=("✅ $bot_key: SUCCESS")
        else
            FAILED=$((FAILED + 1))
            ERROR=$(tail -3 /tmp/manage_rs_bots_${bot_key}_$$.log 2>/dev/null | head -1 || echo "Unknown error")
            RESULTS+=("❌ $bot_key: FAILED - $ERROR")
        fi
        rm -f /tmp/manage_rs_bots_${bot_key}_$$.log
    done
...
if [ -z "$SERVICE_NAME" ]; then
    ...
    echo "Available RS bots: ${!SERVICES[*]}"
    echo "Special: 'all' or 'allbots' to operate on all RS bots"
    exit 1
fi
