...
- **What**: replies with a **header** (guild summary + “Categories → Channels”), then **one message per RS category** (Weekly Guides / Upcoming, Daily Schedule, Instore Important). Each channel line is `N. <#id> - …` where the trailing text is the **daily reminder** description for that channel when found, otherwise a **schedule-style** label parsed from the channel name (same rules as `DailyScheduleReminder/schedule_parser.py`, duplicated in `RSAdminBot/review_rs_daily_blurbs.py`). Reminder source: channel `1313575695267663893` (config: `review_rs_listener.daily_reminder_channel_id`), **newest-first** scan for a post containing `TOMORROW'S DAILY REMINDER - MM/DD` where **MM/DD matches today** in `daily_reminder_timezone` (default `America/New_York`). Only lines under **Online Reminders:** and **Instore Reminders:** with `<#channel_id>` are used; optional ` - description` after the mention. Then **one message per configured “important” channel** with the same blurb rule on the header line plus the last 3 message links. If a category list is unusually long, it may be split into multiple messages to stay under Discord’s length limit. Tuning: `daily_reminder_history_limit`, `review_rs_channel_blurb_max_chars`.
