...
- **What**: replies with a **header** (guild summary + “Categories → Channels”), then **one message per RS category** (Weekly Guides / Upcoming, Daily Schedule, Instore Important). Each channel line is `N. <#id> - …` where the trailing text is the **daily reminder** description for that channel when found, otherwise a **schedule-style** label parsed from the channel name (same rules as `DailyScheduleReminder/schedule_parser.py`, duplicated in `RSAdminBot/review_rs_daily_blurbs.py`). Reminder source: channel `1313575695267663893` (config: `review_rs_listener.daily_reminder_channel_id`), **newest-first** scan for a post containing `DAILY REMINDER - MM/DD` where **MM/DD matches today** in `daily_reminder_timezone` (default `America/New_York`). Only lines under **Online Reminders:** and **Instore Reminders:** with `<#channel_id>` are used; optional ` - description` after the mention. Then **one message per configured “important” channel** with the same blurb rule on the header line plus the last 3 message links. If a category list is unusually long, it may be split into multiple messages to stay under Discord’s length limit. Tuning: `daily_reminder_history_limit`, `review_rs_channel_blurb_max_chars`.

#### Free-member RS preview (auto, RS Daily Reminder channel)
- **Trigger**: new post or **edit** in RS channel `1313575695267663893` when body matches `DAILY REMINDER - MM/DD` (`TODAY'S` / `TOMORROW'S` optional).
- **Output**: webhook posts to `1504893057693388870` as **Neo** (`webhook_username` / `webhook_avatar_user_id` in config). Webhook URL: **`review_rs_listener.free_preview_webhook_url`** in `RSAdminBot/config.secrets.json` (not committed).
- **Layout**: intro `# <a:blue_siren:…> DAILY REMINDER - MM/DD`, then three category blocks (bullets, custom titles): **Today's Release Schedule**, **This Week's Upcoming Heat**, **Ongoing Instore Cooks** + **Instore Important**; closing pitch with paid channel + `@everyone`. **No** guild header, **no** “Recent messages (last 3)”.
- **Blurbs**: parsed from **that reminder message** (Online/Instore sections); else schedule-style channel name.
- **Date rollover**: posting a reminder for a **new** `MM/DD` deletes the previous date’s webhook messages in the free channel (state: `RSAdminBot/data/review_rs_free_preview_state.json`). Same-date **edits** replace that date’s suite.
- **Errors**: posted to Neo Test channel `1496065906923540561`. Toggle: `review_rs_listener.free_preview.enabled`.
