...
- **What**: for Reselling Secrets category `1341477669682024588` (configurable), lists **today’s posts** from **midnight** in `instore_daily_timezone` (default `America/New_York`) through **now**, per **text channel** in that category. Skips empty channels and default skip IDs (monitors/chats). **One Discord message per channel** with posts. Header: optional `<@&role>` (set `instore_daily_role_id`), `new leads posted for today **<date>** - <#channel>`, then post count. Each line: `- jump_url - Store - Product` when both **Where:** and a non-label first line parse; otherwise `- jump_url` only. Truncation: `instore_daily_product_max_chars` / `instore_daily_store_max_chars`. Cap: `instore_daily_max_messages_per_channel` (max 500).
